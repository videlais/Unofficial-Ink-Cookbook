...
By default, all of the choice text in your story will appear the same way. You can change the styling of all of the choice text in your story by editing the properties of p.choices in the CSS stylesheet generated by Ink’s "Export for Web" option. You cannot change the properties of individual choices by using this method, however: all of them must use the same styling, which can be somewhat limiting.

Luckily, there is a workaround within Ink that will allow you to style individual choices! This workaround involves using inline CSS within the text of a choice to change the styling of that individual choice. While this workaround is not fully implemented at this time, it will allow you to do things like change the color of individual choices.  Consider the following example:
...
Note that you can see the results of your inline CSS within the Inky editor itself:

[Image of inline CSS in the Inky editor]

## Try It

The examples in this chapter demonstrate how you can use Ink’s web features to create stories that you can easily share on a website. Many of these features rely on knowledge of HTML or CSS that is not fully covered in this book, but this chapter provides some basic examples that should get you started, even if you have never worked with those languages before. While there are plenty of resources online to help you learn HTML and CSS, it is also worth learning how such code works inside the Inky editor, as well as learning about Ink’s specialized web features.
